...
// ---------- HEARTBEAT LED ----------
static const int PIN_HEARTBEAT = 35;
static const uint32_t HEARTBEAT_PERIOD_MS = 5000;
static const uint32_t HEARTBEAT_ON_MS = 100;
...
float readBatteryVolts()
{
  ...
  {
    mv_acc += analogReadMilliVolts(PIN_BATTERY_SENSE);
    delay(2);
  }
...
void loop()
{
  ...
  // Heartbeat: short flash every 5 seconds
  if (!g_heartbeatOn &&
      (now - g_lastHeartbeatMs >= HEARTBEAT_PERIOD_MS))
  {
    digitalWrite(PIN_HEARTBEAT, HIGH);
    g_heartbeatOn = true;
    g_lastHeartbeatMs = now;
    g_heartbeatOffMs = now + HEARTBEAT_ON_MS;
  }

  if (g_heartbeatOn &&
      (int32_t)(now - g_heartbeatOffMs) >= 0)
  {
    digitalWrite(PIN_HEARTBEAT, LOW);
    g_heartbeatOn = false;
  }
...
{
  char line[96];
  if (!readLineFrom(Serial1, line, sizeof(line)))
    return false;
  ...
  if (strstr(line, "MWV"))
  {
    ...
    if (angle && spd && unit)
    {
      dir_deg = atof(angle);
      float v = atof(spd);
      spd_ms = (*unit == 'N') ? v * 0.5144444f : (*unit == 'K') ? v / 3.6f
                                                                : v; // 'M' = m/s
      gust_ms = spd_ms;
      return (dir_deg >= 0 && dir_deg <= 360 && spd_ms >= 0);
    }
